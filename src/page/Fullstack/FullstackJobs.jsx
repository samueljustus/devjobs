...
import { FullstackJobContext } from "../../context/FullstackJobContext";
import { FcGlobe } from "react-icons/fc";
import { TbMoneybag } from "react-icons/tb";
import moment from "moment";
function FullstackJobs() {
  const { fullstackJobData, setFullstackJobData } =
    useContext(FullstackJobContext);
      ...
      <section className="flex flex-col lg:flex-row lg:justify-between lg:items-start lg:gap-10 mt-10">
        <div className=" lg:w-[30%] lg: sticky lg:top-0">
          ...
          <p className="py-3 text-center text-lg text-[#b0b0b0]">
            The latest job listings we've received accross Fullstack Job
            category
          </p>
        </div>
        {fullstackJobData ? (
          <ul className="lg:w-[60%] mt-7 lg:mt-0">
            {fullstackJobData.results.map((data, i) => {
              return (
                <li
                  key={data.id}
